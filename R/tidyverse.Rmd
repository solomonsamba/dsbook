...
In R we can perform a series of operations, for example `select` and then `filter`, by sending the results of one function to another using what is called the _pipe operator_: `|>`. This feature is available starting with  R version 4.1.0, but  before that, the tidyverse used the `%>%` operator made available by the `magrittr` package. Some details are included below. 
...
Suppose we want three summaries from the same variable such as the median, minimum, and maximum heights. The `quantile` function: `quantile(x, c(0.5, 0, 1))` returns the median (50th percentile), the min (0th percentile), and max (100th percentile) of the vector `x`. We can use it with `summarize` like this:
